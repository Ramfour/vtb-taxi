...
>
    <div class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <section class="rounded-[2rem] border border-white/10 bg-white/6 p-6 backdrop-blur">
            ...
            <p class="mt-2 text-sm text-slate-300">Форма пока минимальная, но уже пишет данные в `temp_requests`.</p>
            ...
            <form method="POST" action="{{ route('employee.requests.store') }}" class="mt-6 space-y-4">
                ...
                <label class="block">
                    <span class="mb-2 block text-sm font-medium text-slate-200">Адрес подачи</span>
        ...
        <section class="space-y-6">
            <div class="rounded-[2rem] border border-white/10 bg-white/6 p-6 backdrop-blur">
                ...
                <div class="mt-5 space-y-4">
                    ...
                    @empty
                        <p class="rounded-3xl border border-dashed border-white/15 p-6 text-sm text-slate-400">Пока нет заявок в буфере. Отправьте первую заявку через форму слева.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-[2rem] border border-white/10 bg-white/6 p-6 backdrop-blur">
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold text-white">Финальные заявки</h2>
                    <span class="rounded-full border border-white/10 px-3 py-1 text-xs font-semibold text-slate-300">{{ $dashboard['requests']->count() }}</span>
                </div>
                <div class="mt-5 space-y-4">
                    @forelse ($dashboard['requests'] as $request)
                        <article class="rounded-3xl border border-white/10 bg-slate-900/70 p-4">
                            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div>
                                    <p class="font-semibold text-white">{{ $request->full_name }}</p>
                    ...
                    @empty
                        <p class="rounded-3xl border border-dashed border-white/15 p-6 text-sm text-slate-400">После финализации руководителем заявки появятся здесь.</p>
                    @endforelse
                </div>
            </div>
        </section>
