        ...
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <header class="mb-8 flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-white/6 p-6 shadow-2xl shadow-black/20 backdrop-blur">
                ...
                <nav class="flex flex-wrap gap-3 text-sm">
                    ...
                    @isset($currentUser)
                        <a class="rounded-full border border-white/10 px-4 py-2 text-slate-200 transition hover:border-cyan-300/50 hover:bg-cyan-400/10" href="{{ route('employee.requests.index', ['user_id' => $currentUser->id]) }}">Кабинет сотрудника</a>
                        @if (in_array($currentUser->role->name, ['Manager', 'Admin'], true))
                            <a class="rounded-full border border-white/10 px-4 py-2 text-slate-200 transition hover:border-amber-300/50 hover:bg-amber-400/10" href="{{ route('manager.requests.index', ['user_id' => $currentUser->id]) }}">Кабинет руководителя</a>
                        @endif
                    @endisset
                </nav>
            </header>
